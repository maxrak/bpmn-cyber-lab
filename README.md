...
- **Security Assessment** — annotare processi con analisi dei rischi STRIDE/MITRE
- **Threat Modeling** — identificare minacce su workflow aziendali
- **Didattica BPMN** — insegnare modellazione corretta attraverso anti-pattern interattivi
- **Analisi IAM** — valutare rischi nei processi di Identity & Access Management
- **Audit e Compliance** — documentare controlli di sicurezza sui processi
- **Process Simulation** — verificare il comportamento dei workflow prima del deployment
...
1. Salva il file `.bpmn` in `public/BPMNs/`
2. Esegui `npm run generate-list`
3. Il diagramma comparirà automaticamente nel menu File

### Personalizzare le proprietà cybersecurity

Modifica `src/cyberPropertiesProvider.js` per aggiungere nuovi campi al pannello laterale.

### Modificare la logica di colorazione

Edita `src/riskColorBehavior.js` per cambiare soglie o colori per livello di rischio.

---

## 🐛 Troubleshooting

**Dev server non si avvia / porta occupata**
```bash
lsof -ti:8080 | xargs kill -9 2>/dev/null; npm start
```

**Menu File vuoto o diagrammi mancanti**
```bash
npm run generate-list
```

**Toolbar coperta dalla navbar**  
Svuota la cache del browser (`Cmd+Shift+R` su Mac, `Ctrl+Shift+R` su Windows/Linux).

**Errori JavaScript**  
Apri la console sviluppatore (F12) e verifica eventuali errori di caricamento dei bundle webpack.
