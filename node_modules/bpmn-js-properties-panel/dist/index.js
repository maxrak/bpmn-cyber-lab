...
function hasProcessRef$3(element) {
  return ModelUtil.is(element, 'bpmn:Participant') && getBusinessObject(element).get('processRef');
}
...
function Executable(props) {
  ...
  getValue = element => {
    return getBusinessObject(element).isExecutable;
  };

  // handle properties on processRef level for participants
  if (ModelUtil.is(element, 'bpmn:Participant')) {
    const process = getBusinessObject(element).get('processRef');
    setValue = value => {
      commandStack.execute('element.updateModdleProperties', {
        element,
...
function Name$3(props) {
  ...
  if (ModelUtil.is(element, 'bpmn:TextAnnotation')) {
    options = {
      ...
      getValue: element => {
        return getBusinessObject(element).text;
      }
    };
  }
...
function ExecutionListenerProps({
  ...
  const bpmnFactory = injector.get('bpmnFactory'),
    commandStack = injector.get('commandStack');
  if (ModelUtil.is(element, 'bpmn:Participant') && !getBusinessObject(element).processRef) {
    return;
  }
